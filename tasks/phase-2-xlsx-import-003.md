...
function renderImportPanel() {
  ...
  const summary = p ? `
    <div class="import-summary">
      ...
      <span class="ok">${p.summary.valid_rows} valid</span> ·
      <span class="bad">${p.summary.invalid_rows} invalid</span>
    </div>` : '';
  const invalid = (p && p.invalid_rows.length) ? `
    <h3 class="import-h">Invalid rows (${p.invalid_rows.length}) — not importable</h3>
    <div class="table-scroll"><table><thead><tr><th>Row</th><th>Errors</th></tr></thead><tbody>
      ${p.invalid_rows.map(r => `<tr><td>${r.row_number}</td><td>${esc(r.errors.join('; '))}</td></tr>`).join('')}
    </tbody></table></div>` : '';
  const validPreview = (p && p.valid_rows.length) ? `
    <h3 class="import-h">Valid rows preview (first 10 of ${p.valid_rows.length})</h3>
    <div class="table-scroll"><table><thead><tr>
      <th>Owner</th><th>Track</th><th>Title</th><th>Status</th><th>Type</th>
    </tr></thead><tbody>
      ${p.valid_rows.slice(0, 10).map(r => `<tr>
        <td>${esc(r.owner)}</td><td>${esc(r.track)}</td><td>${esc(r.title)}</td>
        <td>${esc(r.status)}</td><td>${esc(r.type)}</td>
      </tr>`).join('')}
    </tbody></table></div>` : '';
  const commitDisabled = !(p && p.valid_rows.length > 0) ? ' disabled' : '';
  return `
    <div class="import-panel">
      <h2 class="users-title">Import from XLSX</h2>
      <p class="import-note">Admin only. Preview validates against canonical tracks and statuses; nothing is written until you commit. The database is the source of truth — this is a one-time import, not a sync.</p>
...
function bindImportActions() {
  ...
  if (commitBtn) commitBtn.onclick = async () => {
    setErr('');
    const p = state.importPreview;
    if (!p || !p.valid_rows.length) { setErr('Nothing to import — run a preview with valid rows first.'); return; }
    if (!confirm(`Import ${p.valid_rows.length} valid row(s) into the database?`)) return;
    try {
      const res = await api('/import/commit', { method: 'POST', body: { rows: p.valid_rows } });
      state.importPreview = null;
      state.page = 'rows';
      state.workspace = 'all';
      await loadRows();
